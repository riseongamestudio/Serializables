...
namespace RiseOn.Serializables {
    /// <summary>
    /// One saved key of an <see cref="EnumMap{TKey,TValue}"/>: the enum value and its name at the time it was
    /// written, so the map can still find it after the enum changes.
    /// </summary>
    [Serializable]
